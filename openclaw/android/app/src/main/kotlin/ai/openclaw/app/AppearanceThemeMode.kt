...
enum class AppearanceThemeMode(
  ...
  companion object {
    fun fromRawValue(value: String?): AppearanceThemeMode =
      entries.firstOrNull { it.rawValue == value?.trim()?.lowercase() } ?: System

    fun fromDisplayLabel(label: String): AppearanceThemeMode =
      entries.firstOrNull { it.displayLabel.equals(label.trim(), ignoreCase = true) } ?: System
  }
}
