...
description: |
  ...
  Voice triggers (speech-to-text aliases): "make a video", "render this as video", "produce a reel".
triggers:
  - make a video
...
<body>
  <div id="stage"
    ...
    <!-- Background video: plays full composition (track 0 = bottom layer) -->
    <video id="bg"
           data-start="0"
           data-duration="10"
           data-track-index="0"
    ...
    <!-- Text title: appears at t=1s for 4 seconds (track 1 = above video) -->
    <h1 id="title"
        data-start="1"
        data-duration="4"
        data-track-index="1"
    ...
    <!-- Logo: fades in at t=5s for 3 seconds (track 2 = topmost visual) -->
    <img id="logo"
         data-start="5"
         data-duration="3"
         data-track-index="2"
    ...
    <!-- Background audio track (volume 0.6 so narration can sit on top) -->
    <audio id="music"
           data-start="0"
           data-duration="10"
           data-track-index="3"
...
| `data-track-index` | ✗ | Layer index for audio/video tracks (0-based; controls stacking order) |
| `data-volume` | ✗ | Audio volume 0–1 (audio/video elements only; default: 1) |
| `data-adapter` | ✗ | Animation adapter: `gsap`, `lottie`, `css`, `anime`, `three` |

---

## Step 3: Inspect and Lint

**Inspect** composition metadata (dimensions, duration, element count):

```bash
npx hyperframes inspect index.html
```

**Lint** before committing to a full render:
...
HyperFrames ships a catalog of pre-built blocks (transitions, captions, overlays,
charts). Install one into your project:

```bash
npx hyperframes add fade-transition
npx hyperframes add lower-third
npx hyperframes add progress-bar
```

This copies the block's HTML snippet and assets into the project. Include the
snippet in your composition and wire up `data-start` / `data-duration` as needed.

---

## GSAP Animations

GSAP timelines must be registered on `window.__timelines[compositionId]` so the
renderer can seek to any frame without linear playback:
...
<script>
  ...
  tl.from("#logo",  { opacity: 0, scale: 0.8, duration: 0.5 }, 5);

  window.__timelines = window.__timelines || {};
  window.__timelines["my-video"] = tl;   // must match data-composition-id
</script>
```

Key rule: the GSAP timeline must be `paused: true` and keyed by `compositionId`.
The renderer controls playback time; never call `tl.play()`.
...
- `/design-shotgun` → `/design-html` → `/hyperframes`: Full pipeline from concept
  to rendered video.
- `reels-html/`: The browser-based FFmpeg-WASM editor in this repo is an alternative
  for interactive editing. HyperFrames is the CLI/agent-native headless path.
...
- npm: `hyperframes` (latest; Node.js 22+ required)
