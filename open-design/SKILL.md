...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"open-design","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
If `HOME_CLONE_MISSING` or after cloning:
```bash
git clone https://github.com/nexu-io/open-design.git ~/open-design
```

Show the last 3 commits so the user knows what version they have:
```bash
...
Options (show only relevant ones based on CLI detection above):
...
If B: prompt via AskUserQuestion ("Paste your Anthropic API key — it won't be echoed:"),
then write `ANTHROPIC_API_KEY=<value>` to `~/open-design/.env`.
If C: prompt for key and base URL, write to `~/open-design/.env`.
If D: show `cat ~/open-design/.env.example` and continue.

**NEVER print API key values back to the user. Only confirm: "Key saved to ~/open-design/.env".**
...
If a port is in use by a non-Open-Design process, ask the user before killing it.
...
If `DAEMON_TIMEOUT`, show `tail -30 /tmp/od-daemon.log` and stop with BLOCKED status.
...
echo "WEB_PID: $!"
# Wait up to 30s for web to be ready
for i in $(seq 1 30); do
...
If `WEB_TIMEOUT`, show `tail -30 /tmp/od-web.log` and stop.
...
Write configuration to CLAUDE.md (read file first, then append or replace the section):
...
If a `## Skill routing` section exists in CLAUDE.md, append:
```
