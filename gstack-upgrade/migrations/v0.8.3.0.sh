#!/usr/bin/env bash
# Migration: v0.8.3.0 — Remove stale /debug skill installs
#
# Claude Code ships a built-in /debug command that was shadowing the gstack
# /debug skill. In v0.8.3 the gstack skill was renamed to /investigate to
# remove the shadow. This migration removes old on-disk /debug installs so
# Claude Code's native /debug is no longer shadowed.
#
...
# ~/.claude/skills/gstack/. A user's own /debug skill (regular file,
# or symlink pointing elsewhere) is preserved.
#
# Three supported install shapes to handle:
#   1. ~/.claude/skills/debug is a directory symlink into gstack.
#   2. ~/.claude/skills/debug is a regular directory whose ONLY file
...
if [ -z "${HOME:-}" ]; then
  echo "  [v0.8.3.0] HOME is unset or empty — skipping migration." >&2
  exit 0
fi

SKILLS_DIR="${HOME}/.claude/skills"
OLD_TOPLEVEL="${SKILLS_DIR}/debug"
OLD_NAMESPACED="${SKILLS_DIR}/gstack/debug"
GSTACK_ROOT_REAL=""
...
# --- Shape 1: top-level ~/.claude/skills/debug
if [ -L "$OLD_TOPLEVEL" ]; then
  ...
  if [ -n "$GSTACK_ROOT_REAL" ] && path_inside "$target_real" "$GSTACK_ROOT_REAL"; then
    rm -- "$OLD_TOPLEVEL"
    echo "  [v0.8.3.0] Removed stale /debug symlink (was shadowing Claude Code's built-in /debug command)."
    removed_any=1
  else
    echo "  [v0.8.3.0] Leaving $OLD_TOPLEVEL alone — symlink target is outside gstack (or unresolvable)."
  fi
  ...
  if [ "$file_count" = "0" ] && [ "$symlink_count" = "1" ] && [ -L "$OLD_TOPLEVEL/SKILL.md" ]; then
    ...
    if [ -n "$GSTACK_ROOT_REAL" ] && path_inside "$target_real" "$GSTACK_ROOT_REAL"; then
      ...
      rm -r -- "$OLD_TOPLEVEL"
      echo "  [v0.8.3.0] Removed stale /debug install directory (gstack prefix-mode)."
      removed_any=1
    else
      echo "  [v0.8.3.0] Leaving $OLD_TOPLEVEL alone — SKILL.md symlink target is outside gstack."
    fi
  else
    echo "  [v0.8.3.0] Leaving $OLD_TOPLEVEL alone — not a gstack-owned install (has custom content)."
  fi
fi
# Missing → no-op (idempotency).

# --- Shape 2: ~/.claude/skills/gstack/debug/
...
if [ -L "$OLD_NAMESPACED" ]; then
  ...
  if [ -n "$GSTACK_ROOT_REAL" ] && path_inside "$target_real" "$GSTACK_ROOT_REAL"; then
    rm -- "$OLD_NAMESPACED"
    echo "  [v0.8.3.0] Removed stale ~/.claude/skills/gstack/debug symlink."
    removed_any=1
  else
    echo "  [v0.8.3.0] Leaving $OLD_NAMESPACED alone — symlink target is outside gstack."
  fi
  ...
  if [ -n "$GSTACK_ROOT_REAL" ] && path_inside "$target_real" "$GSTACK_ROOT_REAL"; then
    ...
    echo "  [v0.8.3.0] Removed stale ~/.claude/skills/gstack/debug/ (replaced by /investigate)."
    removed_any=1
  else
    echo "  [v0.8.3.0] Leaving $OLD_NAMESPACED alone — resolves outside gstack."
  fi
fi

if [ "$removed_any" = "1" ]; then
  echo "  [v0.8.3.0] /debug is now Claude Code's built-in debugger. Use /investigate for systematic root-cause analysis."
fi
